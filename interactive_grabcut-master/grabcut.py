...
def pathToDownloads(fileName):
    homeDir = os.path.expanduser('~')
    Downloads = os.path.join(homeDir, 'Downloads')
    file_path = os.path.join(Downloads, 'grabcut/')
    if(not os.path.isdir(file_path)):
        os.mkdir(file_path)
    i = 0
    file_path = file_path+'outfit'
    while os.path.isfile(file_path + str(i) + '.png'):
        i += 1
    return file_path + str(i) + '.png'
...
def grabcut_rect(image_path, rect_coords, project_name, row_id):
    ...
    # added on 29/06/22-12:30PMSGT
    cv2.imwrite(pathToDownloads('outfit'), img2_refined)
...
def grabcut_drawing(image_path, rect_coords, fg_drawing, bg_drawing, project_name, row_id):
    ...
    # zero out foreground and background models
    ...
    mask, bgdModel, fgdModel = cv2.grabCut(
        img, mask, rect, bgdModel, fgdModel, 5, cv2.GC_INIT_WITH_MASK)
    mask2 = np.where((mask == 2) | (mask == 0), 0, 1).astype('uint8')
    img2 = img*mask2[:, :, np.newaxis]
